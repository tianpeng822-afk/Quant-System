...
for idx, row in df.iterrows():
    ...
    while next_sell_level < len(LADDER_SELL) and pnl_pct >= LADDER_SELL[next_sell_level][0]:
        ...
        print('%s SELL 止盈+%d%%: 净值=%.4f 卖出=%.0f元 剩余份额=%.2f 剩余成本=%.0f 新成本价=%.4f 新浮盈=%.1f%%' % (dt.strftime('%Y-%m-%d'), int(threshold*100), nav, sell_amount, shares, cost, new_avg, new_pnl))
        break
    ...
    if not sell_triggered:
        while next_buy_level < len(LADDER_BUY) and dd_pct >= LADDER_BUY[next_buy_level][0]:
            ...
            new_avg = cost / shares if shares > 0 else 0
            new_pnl = (nav - new_avg) / new_avg * 100 if new_avg > 0 else 0
            print('%s BUY 回撤%d%%: 净值=%.4f 补仓=%.0f元 份额+%.2f 总份额=%.2f 新成本价=%.4f 新浮盈=%.1f%%' % (dt.strftime('%Y-%m-%d'), int(dd_pct*100), nav, buy_amount, buy_shares, shares, new_avg, new_pnl))
            peak_nav = nav
            break
